...
export class LibraryBooksComponent  implements OnInit {
  ...
  getAllNotLendedLibrary(){
    ...
    this.mastersService.getAllNotLendedLibrary()
    .subscribe((res:any)=>{
  ...
  goToLend(id){
    ...
    dialogRef.afterClosed().subscribe(result => {
      console.log(result);
      if(result=="Yes"){
        this.mastersService.lendBook(id)
        .subscribe((res:any)=>{
          this.alertService.successSnackBar("Book Lended Successfully");
          this.getAllNotLendedLibrary();
        });
      }
    });
